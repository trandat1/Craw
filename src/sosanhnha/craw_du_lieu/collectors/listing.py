...
def collect_list_items(
    ...
    for _ in range(20):
        els = wait.until(
            EC.visibility_of_all_elements_located(
                (By.CSS_SELECTOR, ".w-full.border-b.bg-white")
            )
        )
    ...
    for el in els:   # els = danh sách các card .row-item
        try:
            ...
            # ==============================
            try:
                img = el.find_element(By.CSS_SELECTOR, "a.picture img")
            ...
            # ==============================
            try:
                price = el.find_element(By.CSS_SELECTOR, "span.price").text.strip()
            except:
                price = ""

            try:
                area = el.find_element(By.CSS_SELECTOR, "span.area").text.strip()
            except:
                area = ""
